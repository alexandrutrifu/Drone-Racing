...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        secondLightPosition = glm::vec3(1, 1, 1);
...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0, 0.3, 1));
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    float cutoff_angle_loc = glGetUniformLocation(shader->program, "cutoff_angle");
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        // Control second light position using on I, J, K, L, O, U
        ...
        // Control light direction using on T, F, G, H
        if (window->KeyHold(GLFW_KEY_G)) lightDirection -= forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_C)) lightDirection -= right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_V)) lightDirection += forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_B)) lightDirection += right * deltaTime * speed;
        
        lightDirection = glm::normalize(lightDirection);

        glm::vec3 a = glm::rotate(glm::mat4(1), glm::radians(30.0f), glm::vec3(1, 0, 0)) * glm::vec4(0, 1, 0, 0);

        // TODO(student): Set any other keys that you might need
        if (window->KeyHold(GLFW_KEY_Z))
            cutoff_angle -= deltaTime * 60;  
